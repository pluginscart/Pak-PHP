...
namespace BrowserApplicationFramework;
 ...
 * @package    BrowserApplicationFramework
 * @author     Nadir Latif <nadir@pakjiddat.com>
 * @license    https://www.gnu.org/licenses/gpl-2.0.html GNU General Public License, version 2
 * @version    Release: 1.0.0
 * @link       N.A
 */
abstract class BrowserApplication
{    
    /**
        ...
        if (!isset($configuration['application_url_mappings'][$configuration['option']]))
			{
				...
				$class_function="Handle".ucwords(str_replace("_"," ",$configuration['option']));
				...
				if (is_callable($application_callback))
					{
						$configuration['application_url_mappings'][$configuration['option']]=array(
			                "controller" => array(
			                    "object_name" => "application",
			                    "function_name" => $class_function
			                )
			            );
						ApplicationConfiguration::SetConfig("application_url_mappings",$configuration['application_url_mappings']);
					}
				else throw new \Exception("Invalid url request sent to application", 110);
			}            
            ...
            if (!$configuration['testing']['test_mode']) {
                ...
                if (isset($configuration['application_url_mappings'][$configuration['option']]['controller'])) {
                    /** The controller function is run **/
                    $response = $this->RunControllerFunction($configuration['option']);
                ...
                else if (isset($configuration['application_url_mappings'][$configuration['option']]['templates'])) {
                    /** The application template is rendered with parameters generated by presentation class function **/
                    $template_contents = $this->RenderApplicationTemplate($configuration['option']);
                ...
                /** If no controller and no template is defined for the current url option then an exception is thrown **/
                    ...
                    throw new \Exception("No controller or template defined for the current url.", 110);
            }
    ...
    {        
        $template_contents = ApplicationConfiguration::GetComponent("template")->Render("root", array());
    ...
    {        
        ...
        $application_url_mappings = ApplicationConfiguration::GetConfig('application_url_mappings');
        $controller_object_name   = $application_url_mappings[$option]['controller']['object_name'];
        $controller_object        = ApplicationConfiguration::GetComponent($controller_object_name);
        $function_name            = $application_url_mappings[$option]['controller']['function_name'];
        $response                 = $controller_object->$function_name($option);
        return $response;        
    }    
    /**
    ...
    {        
        ...
        if (strlen($encoded_parameters) > 150)
            throw new \Exception("The size of the encoded parameters must be less than 100 characters", 110);
        ...
        $web_application_url   = $default_configuration['web_application_url'];
        
        /** The encoded url is created and returned **/
        if ($is_link)
            $encoded_url = $web_application_url . "?option=" . $option . '&amp;parameters=' . ($encoded_parameters);
        else
            $encoded_url = $web_application_url . "?option=" . $option . '&parameters=' . ($encoded_parameters);
