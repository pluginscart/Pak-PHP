...
class ExampleClass
{
    ...
    public function ErrorHandlingTest()
    {
        ...
        $errorhandling_obj = \UtilitiesFramework\UtilitiesFramework::Factory("errorhandler", $parameters);
    ...
    public function FileSystemTest()
    {
        ...
        /** The new line characters are removed for better viewing **/
        $html_content = str_replace("\r", "", $html_content);
        $html_content = str_replace("\n", "", $html_content);
        /** The url of the html5 validator **/
        $validator_url = "https://html5.validator.nu/";
		/** The output format. for browser applications this is html **/
        $output_format = ($is_browser_application) ? "html" : "text";
        $show_source   = ($is_browser_application) ? "yes" : "no";
        /** The parameters that will be sent to the W3C validation service **/
        $content = array(
            "parser" => "html5",
            "out" => $output_format,
        ...
        /** The http headers that will be sent **/
        $headers = array(
            "Content-type: multipart/form-data; boundary=---------------------------" . strlen($html_content)
        );
        /** The file sytem object is fetched. It contains the curl based method for validating the html **/
        $errorhandling_obj = \UtilitiesFramework\UtilitiesFramework::Factory("filesystem");
        /** The html content is posted to the html validator using the file system object function **/
        $validation_results = $errorhandling_obj->GetFileContent($validator_url, "POST", $content, $headers);
        /** The css file name is updated so the validation results can be viewed more easily **/
